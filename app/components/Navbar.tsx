...
const Navbar = () => {
  ...
  return (
    <header className="w-full mx-auto px-8 shadow-lg top-0 z-20 sm:px-20 dark:border-b dark:border-gray-200">
          ...
          <div className="items-center justify-center space-y-8 md:flex md:space-x-6 md:space-y-0">
            {NAV_ITEMS.map((item, i) => (
              <Link
                ...
                className="block lg:inline-block text-black-800 cursor-pointer hover:text-neutral-500"
              >
                {item.label}
              </Link>
            ))}
            {currentTheme === "dark" ? (
              <button
                className="bg-slate-100 px-2 rounded-xl"
